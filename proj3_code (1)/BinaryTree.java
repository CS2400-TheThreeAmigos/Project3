public class BinaryTree<T> implements BinaryTreeInterface<T>
{
   ...
   private int getHeight(BinaryNode<T> node)
   {
       int height = 0;
       if (node != null)
          height = Math.max(getHeight(node.getLeftChild()), getHeight(node.getRightChild())) + 1;
       return height;
   } // end getHeight
   ...
   {
      int lc = 0;//left count
      int rc = 0;//right count
      if(node.getLeftChild()!= null)
      {
         return getNumberOfNodes(node.getLeftChild());
      }
      if(node.getRightChild() != null)
      {
         return getNumberOfNodes(node.getRightChild());
      }
      return lc + rc + 1;
   } // end getNumberOfNodes
